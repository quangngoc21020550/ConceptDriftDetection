...
class EmbeddingNet(nn.Module):
    ...
    def forward(self,datax,datay):
        ...
        loss, class_acc ,centroid_matrix=self.prototypical_loss(input,datay,self.Ns)

        return loss,class_acc,centroid_matrix

    def forward_test(self, datax, centroid_matrix):
        # Get input data (center of mass)
        input = self.PrototypicalNet(datax)
        y_pred = torch.t(cosine_similarity(centroid_matrix, input))
        y_pred = F.softmax(y_pred, dim=1, dtype=torch.float16)
    ...
    def prototypical_loss(self,input, target, n_support):
        ...
        cos_sim = torch.t(cosine_similarity(prototypes, query_samples))
        # log_p_y = F.log_softmax(-dists, dim=1).view(n_classes, n_query, -1)
        log_p_y = F.log_softmax(cos_sim, dim=1).view(n_classes, n_query, -1)
