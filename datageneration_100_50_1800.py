...
def generateAbruptDriftStream(max_samples, first_func, second_func, random_state, drift_pos, window_len):
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        if n_samples==drift_pos+50:
            resultList.append([correct_cnt / iter_n_samples,1])
        else:
            resultList.append([correct_cnt / iter_n_samples,0])
        file_name = "./input/Data/drift-100-50-1800/abrupt/AGRAWALGenerator_"+str(first_func)+\
...
def generateAbruptDriftStream_plus(max_samples, first_func, second_func, first_random_state, second_random_state,
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        file_name = "./input/Data/drift-100-50-1800/abrupt/SEAGenerator_" + str(first_func) + \
...
def generateGradualDriftStream(max_samples, first_func, second_func,
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        if n_samples==drift_pos+50:
            ...
            resultList.append([correct_cnt / iter_n_samples,0])
        # resultList.append(correct_cnt / iter_n_samples)
        file_name = "./input/Data/drift-100-50-1800/gradual/SEAGenerator_"+str(first_func)+\
                    "_"+str(second_func)+"_"+str(first_random_state)+"_"+\
                    str(second_random_state)+"_"+str(all_random_state)\
                    +"_" + str(drift_pos)+ ".csv"
        DataFrame(resultList).to_csv(file_name)
...
def generateIncrementalDriftStream(max_samples, random_state, first_mag_change, second_mag_change,
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        if n_samples == drift_pos + 50:
            ...
            resultList.append([correct_cnt / iter_n_samples, 0])
        # resultList.append(correct_cnt / iter_n_samples)
        file_name = "./input/Data/drift-100-50-1800/incremental/HyperplaneGenerator_"+str(random_state)+\
...
def generateNormalStream(max_samples, GeneratorType, random_state, window_len):
    ...
    if GeneratorType == 0:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-100-50-1800/normal/AGRAWALGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
    ...
    if GeneratorType == 1:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-100-50-1800/normal/HyperplaneGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
            print("Normal processing has been completed:", file_name)
    ...
    if GeneratorType == 2:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-100-50-1800/normal/SEAGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
            print("Normal processing has been completed:", file_name)
    ...
    if GeneratorType == 3:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-100-50-1800/normal/LEDGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
            print("Normal processing has been completed:", file_name)
...
if __name__ == "__main__":
    ...
    for r in  range(100):
        if r%1==0:
            if r>=5 and r <95:
                position.append(r)
    print(len(position))
    # Abrupt Drift (90*2*8=1440)(4*1*90=360) 1800
    for i in range(2, 10):
        first_func = i
        for j in range(2):
            second_func = j
        ...
        for first_random_state in range(100, 101):  # before(100, 110)
            for second_random_state in range(100, 101):  # before(100, 107)
                for random_state in range(len(position)):
                    drift_pos = position[random_state] * 50
   ...
   # Incremental Drift(90*10*2) 1800
    for i in range(1):
        random_state = i
        for j in range(10):
    ...
    for GeneratorType in range(0,5):
        for random_state in range(1,451):#before (0,350)
