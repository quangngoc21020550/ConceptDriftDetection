...
def generateAbruptDriftStream(max_samples, first_func, second_func, random_state, drift_pos, window_len):
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        file_name = "./input/Data/drift-200-1-3800/abrupt/AGRAWALGenerator_"+str(first_func)+\
...
def generateAbruptDriftStream_plus(max_samples, first_func, second_func, first_random_state, second_random_state,
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        file_name = "./input/Data/drift-200-1-3800/abrupt/SEAGenerator_" + str(first_func) + \
...
def generateGradualDriftStream(max_samples, first_func, second_func,
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        if n_samples==drift_pos+1:
            ...
            resultList.append([correct_cnt / iter_n_samples,0])
        # resultList.append(correct_cnt / iter_n_samples)
        file_name = "./input/Data/drift-200-1-3800/gradual/SEAGenerator_"+str(first_func)+\
                    "_"+str(second_func)+"_"+str(first_random_state)+"_"+\
                    str(second_random_state)+"_"+str(all_random_state)\
                    +"_" + str(drift_pos)+ ".csv"
        DataFrame(resultList).to_csv(file_name)
...
def generateIncrementalDriftStream(max_samples, random_state, first_mag_change, second_mag_change,
    ...
    while n_samples < max_samples and stream.has_more_samples():
        ...
        if n_samples == drift_pos + 1:
            ...
            resultList.append([correct_cnt / iter_n_samples, 0])
        # resultList.append(correct_cnt / iter_n_samples)
        file_name = "./input/Data/drift-200-1-3800/incremental/HyperplaneGenerator_"+str(random_state)+\
...
def generateNormalStream(max_samples, GeneratorType, random_state, window_len):
    ...
    if GeneratorType == 0:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-200-1-3800/normal/AGRAWALGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
    ...
    if GeneratorType == 1:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-200-1-3800/normal/HyperplaneGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
            print("Normal processing has been completed:", file_name)
    ...
    if GeneratorType == 2:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-200-1-3800/normal/SEAGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
            print("Normal processing has been completed:", file_name)
    ...
    if GeneratorType == 3:
        ...
        while n_samples < max_samples and stream.has_more_samples():
            ...
            resultList.append(correct_cnt / iter_n_samples)
            file_name = "./input/Data/drift-200-1-3800/normal/LEDGenerator"+str(random_state)+".csv"
            DataFrame(resultList).to_csv(file_name)
            print("Normal processing has been completed:", file_name)
    ...
    for r in  range(200):
        if r%1==0:
            if r>=5 and r <195:
                position.append(r)
    print(len(position))
    #Abrupt Drift (8*2*190=3040)(4*1*190=760) 3800
    for i in range(2, 10):
        first_func = i
        for j in range(2):
            second_func = j
